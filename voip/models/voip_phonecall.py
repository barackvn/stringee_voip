...
class VoipPhonecall(models.Model):
    ...
    state = fields.Selection([('pending', 'Not Held'), ('cancel', 'Cancelled'), ('open', 'To Do'), ('done', 'Held'), ],
                             string='Status', default='open', help='The status is set to To Do, when a call is created.\n'
                                                                   'When the call is over, the status is set to Held.\n'
                                                                   'If the call is not applicable anymore, the status can be set to Cancelled.')
    type = fields.Selection([('incoming', 'Incoming'), ('outgoing', 'Outgoing')], string='Type', default='outgoing')
    call_id = fields.Char('Call ID')

    @api.multi
    def call_success(self, call_id):
        self.ensure_one()
    ...
    def hangup_call(self):
        ...
        self.write({'state': 'done', 'duration': duration, 'note': note, })
        return
    ...
    @api.one
    def get_info(self):
        info = {'id': self.id, 'name': self.name, 'state': self.state, 'date_deadline': self.date_deadline,
                'call_date': self.call_date, 'duration': self.duration, 'phone': self.phone, 'mobile': self.mobile,
                'note': self.note, }
        if self.partner_id:
            ir_model = self.env['ir.model'].search([('model', '=', 'res.partner')])
            info.update(partner_id=self.partner_id.id, activity_res_id=self.partner_id.id,
                        activity_res_model='res.partner', activity_model_name=ir_model.display_name,
                        partner_name=self.partner_id.name, partner_image_small=self.partner_id.image_small,
                        partner_email=self.partner_id.email)
        if self.activity_id:
            ir_model = self.env['ir.model'].search([('model', '=', self.activity_id.res_model)])
            info.update(activity_id=self.activity_id.id, activity_res_id=self.activity_id.res_id,
                        activity_res_model=self.activity_id.res_model, activity_model_name=ir_model.display_name,
                        activity_summary=self.activity_id.summary, activity_note=self.activity_id.note, )
        elif self.mail_message_id:
            ir_model = self.env['ir.model'].search([('model', '=', self.mail_message_id.model)])
            info.update(activity_res_id=self.mail_message_id.res_id, activity_res_model=self.mail_message_id.model,
    ...
    def get_next_activities_list(self):
        return self.search(
            ['|', ('activity_id', '!=', False), ('mail_message_id', '!=', False), ('in_queue', '=', True),
             ('user_id', '=', self.env.user.id), ('date_deadline', '<=', fields.Date.today())],
            order='sequence,date_deadline,id').get_info()

    @api.model
    def get_recent_list(self, search_expr=None, offset=0, limit=None):
        domain = [('user_id', '=', self.env.user.id), ('call_date', '!=', False), ('in_queue', '=', True), ]
        if search_expr:
    ...
    def create_from_contact(self, partner_id):
        ...
        phonecall = self.create(
            {'name': partner.name, 'phone': partner.sanitized_phone, 'mobile': partner.sanitized_mobile,
    ...
    def create_from_recent(self, phonecall_id):
        ...
        phonecall = self.create(
            {'name': recent_phonecall.name, 'phone': recent_phonecall.phone, 'mobile': recent_phonecall.mobile,
    ...
    def create_from_number(self, number):
        ...
        phonecall = self.create({'name': name, 'phone': number, })
        phonecall.init_call()
        return phonecall.get_info()[0]
    ...
    def create_from_incoming_call(self, number, partner_id=False):
        ...
        phonecall = self.create({'name': name, 'phone': number, 'type': 'incoming', 'partner_id': partner_id, })
        phonecall.init_call()
        return phonecall.get_info()[0]
    ...
    def create_from_activity(self, activity):
        ...
        return self.create({'name': activity.res_name, 'user_id': activity.user_id.id, 'partner_id': partner_id,
                            'activity_id': activity.id, 'date_deadline': activity.date_deadline, 'state': 'open',
                            'phone': activity.phone, 'mobile': activity.mobile, 'note': activity.note, })
    ...
    def create_from_phone_widget(self, model, res_id, number):
        ...
        phonecall = self.create({'name': name, 'phone': number, 'partner_id': partner_id, })
        phonecall.init_call()
        return phonecall.get_info()[0]
